...
#endif;
#IFNDEF LASTH__TX;
Constant LASTH__TX = "^^^[Назад]^";
#ENDIF;
#IFNDEF HINTD__TX;
Constant HINTD__TX = "Подсказки отключены.";
#ENDIF;
#IFNDEF WaitForKey;
[ WaitForKey str i;
if (str==0) str="(Пожалуйста, нажмите любую клавишу)";
print (string) str;
@read_char 1 0 0 i;
];
#EndIf;
...
#IFNDEF Helpsub;
[ HelpSub;
		give hint_menu general;
        if (hint_menu hasnt general)
		{print "[Внимание: Меню, которое вы вызываете, помимо информации об \
		игре и авторе, содержит подсказки, которые могут испортить впечатление \
		от игры. Если вы уверены, что не поддадитесь соблазну читать их в любой \
		затруднительной ситуации в игре, введите @<<инфо@>> ещё раз.]^"; give hint_menu general;
       rtrue;}
        if (hint_menu has locked)
                print_ret "Подсказки отключены.";
hint_menu.select();
];
#ENDIF;
[ HelpOffSub;
...
Class HintTopic 
        ...
        with    printsn [; print (name) self;],
                ...
                @set_cursor 3 2; print "ENTER = следующая подсказка"; 
                ...
                do { @read_char 1 0 0 j; } until (j == 'Q' or 'q' or 'й' or 'Й' or 27 
                                                 or 10 or 13 or 132);
                if (j== 'Q' or 'q' or 'й' or 'Й' or 27) return 2;
                ...
                do { @read_char 1 0 0 j; } until (j == 'Q' or 'q' or 'й' or 'Й' or 27 
                                                 or 10 or 13 or 132);
                if (j== 'Q' or 'q' or 'й' or 'Й' or 27) { give o ~general; return 2;}
                .loopover;
                o=sibling(o);
                new_line;
                } until (i==0);
...
#IFNDEF NO_GRAMMAR;
Verb meta "Help" "Hint" "About" "подсказки" "информация" "инфо" "справка" "помощь"
                                * "откл"                        ->HelpOff
                                *                               ->Help;   
#ENDIF;
